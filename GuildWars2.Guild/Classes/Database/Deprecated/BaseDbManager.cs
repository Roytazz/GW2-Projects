...
namespace GuildWars2Guild.Classes.Database.Deprecated
{
    ...
    {
        protected static void AddEntity(T entity) {
        ...
        protected static void AddUniqueEntity(T entity) {
            using(var db = new GW2DbContext()) {
        ...
        protected static void AddEntity(IEnumerable<T> entities) {
        ...
        protected static IEnumerable<T> GetEntities() {
            using(var db = new GW2DbContext()) {
                return db.Set<T>().ToList().Cast<T>().ToList();
            }
        }

        protected static List<T> GetEntities(Expression<Func<T, bool>> predicate) {
            using(var db = new GW2DbContext()) {
                return db.Set<T>().Where(predicate.Compile()).Cast<T>().ToList();
            }
        }

        protected static List<TResult> GetColumn<TResult>(Expression<Func<T, TResult>> predicate) {
            using(var db = new GW2DbContext()) {
