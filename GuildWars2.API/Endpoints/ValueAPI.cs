    ...
    public static class ValueAPI
    {
        ...
        public static AccountValue GetAccountValue(string APIKey, IEnumerable<Item> knownItems) {
            ...
            AccountValue account = new AccountValue() {
                Bank = GetItemValues(accountInv.Bank, itemListings, items),
                Material = GetItemValues(accountInv.MaterialStorage, itemListings, items),
                Wallet = GetWalletEntries(APIKey),
                GemConversion = GetGemToGoldConversion(),
                OwnSellListings = GetTransactionValues(itemListings, accountInv.OwnSellListings),
                OwnBuyListings = GetTransactionValues(itemListings, accountInv.OwnBuyListings)
            };
